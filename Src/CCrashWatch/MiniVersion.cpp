...
CMiniVersion::CMiniVersion(LPCTSTR lpszPath)
{
	...
	ZeroMemory(m_szPrivateBuild, sizeof(m_szPrivateBuild));
	ZeroMemory(m_szSpecialBuild, sizeof(m_szSpecialBuild));
...
BOOL CMiniVersion::Init()
{
	...
	GetStringInfo(_T("PrivateBuild"),     m_szPrivateBuild);
	GetStringInfo(_T("SpecialBuild"),     m_szSpecialBuild);
...
BOOL CMiniVersion::GetPrivateBuild(LPTSTR lpszPrivateBuild, int nSize)
{
	if (!lpszPrivateBuild)
		return FALSE;
	ZeroMemory(lpszPrivateBuild, nSize);
	lstrcpyn(lpszPrivateBuild, m_szPrivateBuild, nSize-1);
	return TRUE;
}

BOOL CMiniVersion::GetSpecialBuild(LPTSTR lpszSpecialBuild, int nSize)
{
	if (!lpszSpecialBuild)
		return FALSE;
	ZeroMemory(lpszSpecialBuild, nSize);
	lstrcpyn(lpszSpecialBuild, m_szSpecialBuild, nSize-1);
	return TRUE;
}
