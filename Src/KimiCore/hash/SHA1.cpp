...
void AlignSHA1(const char *pIn, unsigned int nInLen, UINT64 *pBlks, UINT64 &nBlksLen)
{
    if (pIn == NULL)
    {
        return;
    }
    UINT64 nInLen64 = (UINT64)nInLen;
    UINT64 nBlk = ((nInLen64+8)>>6)+1;
    if (pBlks == NULL)
    {
        nBlksLen = nBlk*16;
        return;
    }
    for (int j=0; j<nInLen64; j++)
    {
        pBlks[j] = 0;
    }
	UINT64 i;
    for(i=0; i<nInLen64; i++)
    {        
        pBlks[i>>2] |= pIn[i]<<(24-(i&3)*8);        
    }
    pBlks[i>>2] |= 0x80<<(24-(i&3)*8);
    pBlks[nBlksLen-1]=(int)(nInLen64*8);
    return ;
}
...
char* Uint64ToHexStr(UINT64 num, char *buf, int nLen)
{
    ...
    char szTemp[9]={0};
    _ultoa_s((unsigned long)num, szTemp, 16);
    static char *s_pZero[]=
    {
        "0",
        "00",
    ...
    int nRealLen = (int)strlen(szTemp);
...
char* SHA1(const unsigned char *pIn, unsigned int nInLen, char *pOut, unsigned int nOutLen)
{        
    if (pIn == NULL)
    {
        return pOut;
    ...
    UINT64 nBlksLen = 0;
    AlignSHA1((const char*)pIn, nInLen, NULL, nBlksLen);
    ...
    AlignSHA1((const char*)pIn, nInLen, pBlks, nBlksLen);    
    UINT64 w[80] ={0};
    UINT64 a=1732584193;
    UINT64 b=-271733879;
    UINT64 c=-1732584194;
