...
/*
TPRICOMMON_API(unsigned long) TL_GetCRC32(unsigned char *szData, unsigned long dwSize) //计算CRC32
{
	if(!b_crc32IninTab)
		Init_CRC32_Table();
...
*/
DWORD get_crc32(const BYTE *szData, DWORD dwSize) //计算CRC32
{
	if(!b_crc32IninTab)
		Init_CRC32_Table();
	DWORD  crc = 0xffffffff;
	int len;
	const BYTE* buffer = szData;
