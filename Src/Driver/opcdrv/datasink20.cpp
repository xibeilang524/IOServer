...
//	TRACE("IKDataSink20::OnDataChange\n");
	...
	((COpcDrv *)pGroup->m_pDrv)->RefreshLastUpdateTime();
	// Loop over items:
	...
	TRACE(_T("IKDataSink20::OnDataChange() 调用 DoMultiVal()\n"));

	((COpcDrv *)pGroup->m_pDrv)->DoMultiVal();//处理剩下的数据
...
STDMETHODIMP IKDataSink20::OnReadComplete (DWORD dwTransID,
	...
	CKGroup *pGroup = NULL;
	CKItem *pItem = NULL;
	TRACE("IKDataSink20::OnReadComplete\n");
	DWORD cdwBadItemHandles = 0; //错误计数，调试用
	...
	if (hGroup					== NULL	||
		...
		pftTimeStamps			== NULL	||
		pErrors					== NULL)
		return (E_INVALIDARG); //检查参数

	pGroup = (CKGroup *) hGroup; //组句柄转换为组对象

	for (DWORD dwItem = 0; dwItem < dwCount; dwItem++)
	{
		pItem = (CKItem *) phClientItems [dwItem]; //数据项句柄转换为数据项对象
		try
		{
			pItem->UpdateData (pvValues [dwItem], pwQualities [dwItem], pftTimeStamps [dwItem]);
		}
	...
	TRACE(_T("IKDataSink20::OnReadComplete() 调用 DoMultiVal()\n"));
