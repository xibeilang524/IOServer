...
STDMETHODIMP_(void) CKAdviseSink::OnDataChange (FORMATETC *pFormatEtc, STGMEDIUM *pMedium)
{
	...
	if (pFormatEtc->cfFormat == CF_DATA_CHANGE)// DATA CHANGE WITHOUT TIMESTAMP NOTIFICATION 不带时标的数据改变通知
	{
		...
		TRACE(_T("CKAdviseSink::OnDataChange( CF_DATA_CHANGE ) 调用 DoMultiVal()\n"));

		if(m_pDrv)
			((COpcDrv *)m_pDrv)->DoMultiVal();//批量处理接收数据
	}

	else if (pFormatEtc->cfFormat == CF_DATA_CHANGE_TIME)// DATA CHANGE WITH TIMESTAMP NOTIFICATION 带时标的数据改变通知
	{
		OPCGROUPHEADER groupheader;
		...
		TRACE(_T("CKAdviseSink::OnDataChange( CF_DATA_CHANGE_TIME ) 调用 DoMultiVal()\n"));
		if(m_pDrv)
			((COpcDrv *)m_pDrv)->DoMultiVal();//批量处理接收数据
	}

	else if (pFormatEtc->cfFormat == CF_WRITE_COMPLETE)// WRITE COMPLETE NOTIFICATION,写完成通知
	{
