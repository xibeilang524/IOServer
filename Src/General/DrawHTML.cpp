...
static void GetNextSymbol(LPCTSTR lpszStart, int nLen, TCHAR &ch, int &nTagLen)
{
	nTagLen = 1;
	...
	int nsize = min(nLen, 6);
	int nPos = 0;
	BOOL bOK = FALSE;
	for (nPos = 0; nPos < nsize; nPos++)
	{
		if (*(lpszStart + nPos) == _T(';'))
		{
			bOK = TRUE;
			break;
		}
	...
	{
	case 3:
		if (*(lpszStart + 1) == _T('g') && *(lpszStart + 2) == _T('t'))
		{
			ch = _T('>');
			nTagLen = 4;
		}
		else if (*(lpszStart + 1) == _T('l') && *(lpszStart + 2) == _T('t'))
		{
			ch = _T('<');
			nTagLen = 4;
		}
		break;
	case 4:
		if (*(lpszStart + 1) == _T('a') && *(lpszStart + 2) == _T('m') && *(lpszStart + 3) == _T('p'))
		{
			ch = _T('&');
			nTagLen = 5;
		}
		break;
	case 5:
		if (*(lpszStart + 1) == _T('n') && *(lpszStart + 2) == _T('b') && *(lpszStart + 3) == _T('s') && *(lpszStart + 4) == _T('p'))
		{
			ch = _T(' ');
			nTagLen = 6;
		}
...
&nbsp;	- 空格
*/
static void GetSymbolText(LPCTSTR lpszStart, int nLen, LPTSTR *lpszText, int *pTextLen)
{
	int nTagLen = 0;
	TCHAR ch;
	LPTSTR szText = NULL;
	LPCTSTR lpszStr = lpszStart;

	if ((nLen <= 0) || (_tcsstr(lpszStart, _T("&")) == NULL))
	{
		*lpszText = (LPTSTR)lpszStart;
		*pTextLen = nLen;
	...
	*pTextLen = 0;
	szText = (LPTSTR)malloc(sizeof(TCHAR) * (nLen + 1));
	*lpszText = szText;
	while(lpszStr < lpszStart + nLen)
	{
		GetNextSymbol(lpszStr, nLen - (int)(lpszStr - lpszStart), ch, nTagLen);
	...
	*/
	...
	for ( ;; ) {
		...
		default:
			...
			// 处理像&nbsp;这样的符号
			GetSymbolText(Start, TokenLength, &lpszText, &nTextLength);

			/* check word length, check whether to wrap around */
			GetTextExtentPoint32(hdc, lpszText, nTextLength, &size);
			if (size.cx > MaxWidth)
			...
			if ((uFormat & DT_CALCRECT) == 0) {
				...
				DrawText(hdc, lpszText, nTextLength, &rc,
