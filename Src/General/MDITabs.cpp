...
#pragma warning(disable : 4312) // 类型转换警告
...
afx_msg LRESULT CMDITabs::OnSizeParent(WPARAM, LPARAM lParam)
{
	...
	{  
		...
		const int height = 24 + (m_bImages ? 1 : 0); // 26 + (m_bImages ? 1 : 0);
		const int offset = 0; // 2
...
BOOL CMDITabs::OnMDIActivate(CWnd *pWnd)
{
	return FALSE;
}
...
void CMDITabs::OnSelChange(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CMDIChildWnd *pChildWnd = static_cast<CMDIChildWnd *>(FromHandle(HWND(item.lParam)));
	if (pChildWnd)
	{
		if (!OnMDIActivate(pChildWnd))
