...
CSize CTrueColorToolBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
CSize CTrueColorToolBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CTrueColorToolBar::_GetButton(int nIndex, TBBUTTON* pButton) const
{
	CTrueColorToolBar* pBar = (CTrueColorToolBar*) this;
...
void CTrueColorToolBar::_SetButton(int nIndex, TBBUTTON* pButton)
{
...
void CTrueColorToolBar::SizeToolBar(TBBUTTON* pData, int nCount, int nLength, BOOL bVert/* = FALSE*/)
{
...
CSize CTrueColorToolBar::CalcLayout(DWORD dwMode, int nLength/* = -1*/)
{
	ASSERT_VALID(this);
...
void CTrueColorToolBar::DrawGripper( CDC & dc )
{ 
	...
	dc.ExcludeClipRect(rcClient);// 去除切客户区
	dc.IntersectClipRect(rcBar); // 包含非客户区

	dc.FillSolidRect(rcBar,::GetSysColor(COLOR_3DFACE));// 擦出背景
	...
	//边框
	dc.Draw3dRect(rtfrm, ::GetSysColor(COLOR_BTNHIGHLIGHT),
		::GetSysColor(COLOR_BTNSHADOW));
}

BOOL CTrueColorToolBar::LoadTrueColorToolBar(int nBtnWidth, UINT uToolBar,	UINT uToolBarHot, UINT uToolBarDisabled)
{
	if (!SetTrueColorToolBar(TB_SETIMAGELIST, uToolBar, nBtnWidth, FALSE, 0))
		return FALSE;

	if (!SetTrueColorToolBar(TB_SETHOTIMAGELIST, uToolBarHot, nBtnWidth, FALSE, 0))
		return FALSE;

	if (uToolBarDisabled) {
		if (!SetTrueColorToolBar(TB_SETDISABLEDIMAGELIST, uToolBarDisabled, nBtnWidth, FALSE, 0))
			return FALSE;
	}
...
BOOL CTrueColorToolBar::LoadTrueColorToolBar(int nBtnWidth, UINT uToolBar,	UINT uToolBarHot, UINT uToolBarDisabled, COLORREF clsMaskColor)
{
	if (!SetTrueColorToolBar(TB_SETIMAGELIST, uToolBar, nBtnWidth, TRUE, clsMaskColor))
		return FALSE;

	if (!SetTrueColorToolBar(TB_SETHOTIMAGELIST, uToolBarHot, nBtnWidth, TRUE, clsMaskColor))
		return FALSE;

	if (uToolBarDisabled) {
		if (!SetTrueColorToolBar(TB_SETDISABLEDIMAGELIST, uToolBarDisabled, nBtnWidth, TRUE, clsMaskColor))
			return FALSE;
	}
...
BOOL CTrueColorToolBar::SetTrueColorToolBar(UINT uToolBarType, UINT uToolBar, int nBtnWidth, BOOL bMaskColor/* = FALSE*/, COLORREF clsMaskColor/* = 0*/)
{
	...
	if (!bMaskColor)
		clsMaskColor = RGB(rgb[0].rgbtRed,
		                   rgb[0].rgbtGreen,
						   rgb[0].rgbtBlue);
	...
	if (cImageList.Add(&cBitmap, clsMaskColor) == -1)
		return FALSE;
