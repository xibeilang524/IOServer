...
#include "resource.h"
#ifndef _SHLOBJ_H_
#pragma message("To avoid this message, please put shlobj.h in your PCH (normally stdafx.h)")
#include <Shlobj.h>
#endif
#ifndef _INC_MALLOC
#pragma message("To avoid this message, please put malloc.h in your PCH (normally stdafx.h)")
...
{
  CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(nIndex);
  ASSERT(pItemData);
...
BOOL CListOptionsCtrl::OnDeleteItem(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(pNMListView->iItem);
  if (pItemData)
    delete pItemData;
...
void CListOptionsCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags )
{
	if (nChar == VK_RIGHT)
	{
      ...
      // if we have a children and VK_RIGHT -> Focus on it !
			CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(nItem);
  ...
  else if (nChar == VK_UP)
  {
    ...
    if (nIndex > 0)
    {
      ...
      CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(nIndex-1);
...
BOOL CListOptionsCtrl::SetBoolean(int nIndex, BOOL bValue)
{
  ...
  CString sText;
  if (bValue)  
    sText.LoadString(IDS_LISTOPTIONS_TRUE);
  else
    sText.LoadString(IDS_LISTOPTIONS_FALSE);
...
void CListOptionsCtrl::PreSubclassWindow() 
{
  ...
  CString sColumn;
  sColumn.LoadString(IDS_LISTOPTIONS_ATTRIBUTE_TEXT);
  sColumn="ÊôÐÔ";
  InsertColumn(0, sColumn, LVCFMT_LEFT, nColumnWidth);
  sColumn.LoadString(IDS_LISTOPTIONS_ATTRIBUTE_VALUE);
  sColumn="Öµ";
...
BOOL CListOptionsCtrl::AddComboBox(int nIndex, CRuntimeClass* pRuntimeClass, DWORD_PTR dwItemData)
{
  ...
  return SetItemData(nIndex, (DWORD_PTR) pItemData);
}
...
void CListOptionsCtrl::CreateNewChildControl(int nIndex)
{
  ...
  if (pItemData->m_pRuntimeClass1->IsDerivedFrom(RUNTIME_CLASS(CListOptionsCombo)))
  {
    ...
    m_pCombo = (CListOptionsCombo*) pItemData->m_pRuntimeClass1->CreateObject();
    ASSERT(m_pCombo);
  ...
  else if (pItemData->m_pRuntimeClass1->IsDerivedFrom(RUNTIME_CLASS(CListOptionsEdit)))
  {
    ...
    m_pEdit = (CListOptionsEdit*) pItemData->m_pRuntimeClass1->CreateObject();
    ASSERT(m_pEdit);
  ...
  else if (pItemData->m_pRuntimeClass1->IsDerivedFrom(RUNTIME_CLASS(CListOptionsDateCtrl)))
  {
    ...
    m_pDateTime = (CListOptionsDateCtrl*) pItemData->m_pRuntimeClass1->CreateObject();
    ASSERT(m_pDateTime);
  ...
  else if (pItemData->m_pRuntimeClass1->IsDerivedFrom(RUNTIME_CLASS(CListOptionsIPAddressCtrl)))
  {
    ...
    m_pIPAddress = (CListOptionsIPAddressCtrl*) pItemData->m_pRuntimeClass1->CreateObject();
    ASSERT(m_pIPAddress);
  ...
  else if (pItemData->m_pRuntimeClass1->IsDerivedFrom(RUNTIME_CLASS(CListOptionsBrowseButton)))
  {
    ...
    m_pButton = (CListOptionsBrowseButton*) pItemData->m_pRuntimeClass1->CreateObject();
    ASSERT(m_pButton);
  ...
  if (pItemData->m_pRuntimeClass2)
  {
    ...
    {
      m_pSpin = (CListOptionsSpinCtrl*) pItemData->m_pRuntimeClass2->CreateObject();
      ASSERT(m_pSpin);
    ...
    else
    {
      ...
      m_pButton = (CListOptionsBrowseButton*) pItemData->m_pRuntimeClass2->CreateObject();
      ASSERT(m_pButton);
...
void CListOptionsCtrl::SetDateTime(int nIndex, const SYSTEMTIME& st)
{
  ...
  //Also update the text while we are at it
  CListOptionsDateCtrl* pTempDateTime = (CListOptionsDateCtrl*) pItemData->m_pRuntimeClass1->CreateObject();
...
void CListOptionsCtrl::SetIPAddress(int nIndex, DWORD dwAddress)
{
  ...
  //Also update the text while we are at it
  CListOptionsIPAddressCtrl* pTempIPAddress = (CListOptionsIPAddressCtrl*) pItemData->m_pRuntimeClass1->CreateObject();
...
BOOL CListOptionsCtrl::OnClick(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
  ...
  if (nFoundIndex != -1)
  {
    ...
    CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(nFoundIndex);
	  if (pItemData && pItemData->m_pRuntimeClass1 && m_nControlItem == -1)
...
BOOL CListOptionsCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  switch (pCustomDraw->nmcd.dwDrawStage) 
  {
    ...
    {
      int nItem = (int) pCustomDraw->nmcd.dwItemSpec;
      CListOptionsItemData* pItemData = (CListOptionsItemData*) GetItemData(nItem);
      if (pItemData)
      {
...
BEGIN_MESSAGE_MAP(CListOptionsFontNameCombo, CListOptionsCombo)
	//{{AFX_MSG_MAP(CListOptionsFontNameCombo)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{ 
  ...
  CListOptionsFontNameCombo* pThis = (CListOptionsFontNameCombo*) lpData;
  ASSERT(pThis);
...
BEGIN_MESSAGE_MAP(CListOptionsBooleanCombo, CListOptionsCombo)
	//{{AFX_MSG_MAP(CListOptionsBooleanCombo)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
IMPLEMENT_DYNCREATE(CListOptionsAlignCombo, CListOptionsCombo)
BEGIN_MESSAGE_MAP(CListOptionsAlignCombo, CListOptionsCombo)
	//{{AFX_MSG_MAP(CListOptionsAlignCombo)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

int CListOptionsAlignCombo::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  //Let the parent class do its thing
	if (CListOptionsCombo::OnCreate(lpCreateStruct) == -1)
		return -1;
...
{
  CString sCaption;
  sCaption.LoadString(IDS_LISTOPTIONS_DEFAULT_FOLDER_CAPTION);
  return sCaption;
}
...
{
  CString sCaption;
  sCaption.LoadString(IDS_LISTOPTIONS_DEFAULT_FILE_CAPTION);
  return sCaption;
}
...
{
  CString sCaption;
  sCaption.LoadString(IDS_LISTOPTIONS_DEFAULT_FILE_EXTENSION_FILTER);
  return sCaption;
}
...
BEGIN_MESSAGE_MAP(CListOptionsSpinCtrl, CSpinButtonCtrl)
	//{{AFX_MSG_MAP(CListOptionsSpinCtrl)
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CListOptionsSpinCtrl::SetListBuddy(CListOptionsCtrl* pListCtrl) 
{ 
  m_pListCtrl = pListCtrl; 
}

void CListOptionsSpinCtrl::SetEditBuddy(CListOptionsEdit* pEdit) 
{ 
  m_pEdit = pEdit; 
}
...
BEGIN_MESSAGE_MAP(CListOptionsBrowseButton, CButton)
	//{{AFX_MSG_MAP(CListOptionsBrowseButton)
	ON_WM_KILLFOCUS()
	ON_CONTROL_REFLECT(BN_CLICKED, OnClicked)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CListOptionsBrowseButton::SetListBuddy(CListOptionsCtrl* pListCtrl) 
{ 
  m_pListCtrl = pListCtrl; 
}

void CListOptionsBrowseButton::SetEditBuddy(CListOptionsEdit* pEdit) 
{ 
  m_pEdit = pEdit; 
}

void CListOptionsBrowseButton::SetComboBuddy(CListOptionsCombo* pCombo)
{
  m_pCombo = pCombo;
}
...
BEGIN_MESSAGE_MAP(CListOptionsFileDialog, CFileDialog)
	//{{AFX_MSG_MAP(CListOptionsFileDialog)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(CListOptionsDateCtrl, CDateTimeCtrl)
	//{{AFX_MSG_MAP(CListOptionsDateCtrl)
	ON_WM_CHAR()
	ON_WM_GETDLGCODE()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
CString CListOptionsDateCtrl::GetDisplayText(const SYSTEMTIME& st)
{
  ...
  while (nChars && !bWantStop)
  {
    ...
    LPTSTR pszBuffer = (LPTSTR) _alloca(nChars * sizeof(TCHAR));
...
BEGIN_MESSAGE_MAP(CListOptionsTimeCtrl, CListOptionsDateCtrl)
	//{{AFX_MSG_MAP(CListOptionsTimeCtrl)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(CListOptionsIPAddressCtrl, CIPAddressCtrl)
	//{{AFX_MSG_MAP(CListOptionsIPAddressCtrl)
	ON_WM_CHAR()
	ON_WM_GETDLGCODE()
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
...
BEGIN_MESSAGE_MAP(CListOptionsHeaderCtrl, CHeaderCtrl)
	//{{AFX_MSG_MAP(CListOptionsHeaderCtrl)
	//}}AFX_MSG_MAP
...
void DDX_ListEdit(CDataExchange* pDX, int nIDC, int nIndex, CString& sText)
{
  ...
  CListOptionsCtrl* pCtrlListOptions = (CListOptionsCtrl*) CWnd::FromHandlePermanent(hWndCtrl);
  ASSERT(pCtrlListOptions);
