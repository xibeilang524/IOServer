...
LONG CUTEdit::OnSetText(UINT wParam, LONG lParam)
{
	...
	{
		LONG result= (LONG)CEdit::Default();
...
int CUTEdit::GetNextInputLocation(int nSelectionStart)
{
	...
	{
		nNextInputLocation=(int)m_arrLeftLiterals.GetSize()+
			(int)m_arrInputData.GetSize();
...
int CUTEdit::GetPreviousInputLocation(int nSelectionStart)
{
	...
	{
		nPreviousInputLocation = (int)m_arrLeftLiterals.GetSize();
		for(int nIndex=nSelectionStart; 
...
int CUTEdit::RPtoLP(int nRealPos)const 
{
	...
	if(nRealPos<m_arrLeftLiterals.GetSize() || 
		...
		return(nRealPos - (int)m_arrLeftLiterals.GetSize());
}
...
int CUTEdit::LPtoRP(int nLogicalPos)const 
{
	if(nLogicalPos<0 || nLogicalPos>m_arrInputData.GetSize())
		...
		return(nLogicalPos + (int)m_arrLeftLiterals.GetSize());
}
...
{
	int nSymbolsBefore = (int)m_arrInputData.GetSize();

	BOOL bSetToNegative=FALSE;
	BOOL bDecimalSeparatorFound=FALSE;
	for(int nIndex = (int)m_arrInputData.GetSize()-1; nIndex>=0; nIndex--)
	{
	...
	if(GetGroupLength()>0)
	{
		...
		nDecimalDelimiterPos=(nDecimalDelimiterPos==-1) ?
			(int)m_arrInputData.GetSize() : nDecimalDelimiterPos;
	...
	int nSymbolsAfter = (int)m_arrInputData.GetSize();
...
int CUTNumericEdit::ChangeSign(BOOL bNegative)
{
	...
	int nLeftLiteralsBefore = (int)m_arrLeftLiterals.GetSize();
	...
	return (int)m_arrLeftLiterals.GetSize() - nLeftLiteralsBefore;
}
...
BOOL CUTNumericEdit::SetValue(double dValue)
{
	...
	int nSymbolCount = (int)strlen(buffer);
									...
									TCHAR chNewChar, BOOL bReplaceExisting)
{
		...
		(m_bForceToUsePeriod && chNewChar==_T('.')))
	{
			...
			(m_bForceToUsePeriod && chNewChar==_T('.')))
		{
			...
			if(nPos==-1)
			{
				nSelectionStart = (int)(m_arrLeftLiterals.GetSize()+m_arrInputData.GetSize());
...
int CUTNumericEdit::GetDecimalDigitsInserted() const
{
	...
	int nDecimalDelimiterPos=FindSymbol(GetDecimalSeparator());
	nDecimalDelimiterPos=(nDecimalDelimiterPos==-1 ? 
		(int)m_arrInputData.GetSize() : RPtoLP(nDecimalDelimiterPos));
